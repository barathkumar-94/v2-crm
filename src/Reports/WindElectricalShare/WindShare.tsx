import * as React from 'react';

import {
  ScrollabeContainer,
  ControlType,
  RetinaFormBuilder,
  RFSection,
  IControlDefinition,
  IRFEventParams,
  TableCellRendererType,
  IRFData,
  usePageQueryParam,
  TableCellEditorType,
} from '@retina360-ai/core-ui-library-v2';
import { IPageBaseProps } from '../../common/objects';
import { truncate } from 'lodash';
import { CRM_BILLING, CRM_MASTER } from '../../common/constants';
import { RFCRMToolbar } from '../../common/components/toolbar';
import { RFFooter } from '../../common/components/footer';
import { DataSection } from '../../master/Rolemaster/managerole';

const searchInputs = ['generationyear','generationperiod','site','generationtype'];
const searchSection: IControlDefinition[] = [
 
  {
    type: ControlType.COMBOBOX,
    name: 'generationyear',
    label: 'Generation Year',
    masterField:'generationyear',
    event :{
      serviceName:'RCRM_REPORT_GENERATION_YEAR_ONCHANGE',
      moduleName :CRM_BILLING,
      input:['generationyear']
      },
      required:true,
  },
  {
    type: ControlType.COMBOBOX,
    name: 'generationperiod',
    label: 'Generation Period',
    masterField:'generationperiod',
    required:true,
  },


  {
    type: ControlType.COMBOBOX,
    name: 'site',
    label: 'Site',
    masterField:'site',
    required:true,
  },

  {
    type: ControlType.COMBOBOX,
    name: 'generationtype',
    label: 'Generation Type',
    masterField:'generationtype',
  
  },
  
  {
    type: ControlType.BUTTON,
    isPrimary: true,
    name: 'Search',
    label: 'Search',
    event: {
      serviceName: 'RCRM_SEARCH_WIND_SOLAR_SHARE',
      moduleName: CRM_BILLING,
      input: [...searchInputs],
    },
  },

];


const tableSection: IControlDefinition[] = [
  {
    type: ControlType.TABLE,
    name: 'WindSolarElectricalGrid',
    isPrimeReactTable:true,
    columns: [
      {
        title: 'Generation Type',
        dataField: 'GENERATION_TYPE',
      },
      {
        title: 'Name of Wind/Solar Farm Owner',
        dataField: 'NAME_OF_WIND_SOLAR_FARM_OWNER',
        
      },
      {
        title: 'Asset ID',
        dataField: 'ASSET_ID',
    
      },

      {
        title: 'Asset Name',
        dataField: 'ASSET_NAME',
    
      },

      {
        title: 'DISCOM Allocation',
        dataField: 'DISCOM_ALLOCATION',
    
      },

      {
        title: 'Installed Capacity(MW)',
        dataField: 'INSTALLED_CAPACITY',
    
      },

      {
        title: 'Share in Active Energy(Mwh)',
        dataField: 'SHARE_IN_ACTIVE_ENERGY',
    
      },

      {
        title: 'Share in Reactive Energy(Mvarh)',
        dataField: 'SHARE_IN_REACTIVE_ENERGY',
    
      },
    ],
  },
];






const onLoadEventParams: IRFEventParams = {
  serviceName: 'RCRM_INIT_SOLAR_WIND_SHARE',
  moduleName: CRM_BILLING,
  input: ['id']
};

export const WindElectricalShareReport: React.FC<IPageBaseProps> = (props) => {
  const { id } = usePageQueryParam();


  const initialData: IRFData = {
    id: id
  };
  return (
    <RetinaFormBuilder initialValues={initialData} scrollKey={props.scrollKey} onLoadEventParams={onLoadEventParams} className={'page-with-footer-btn-meta'}>
      <RFCRMToolbar hasBackButton />
      <ScrollabeContainer hasHeader={true}>
        <RFSection controls={searchSection} title={'General'} columns={6} className={'section-header-bg-primary'} collapse={false} />
        <RFSection controls={tableSection} title={'Wind and Solar Electrical Share'}columns={1} className={'section-header-bg-primary'} collapse={false}/>
             </ScrollabeContainer>
      {/* <RFFooter buttons={actionBarButtons} metaData={DataSection} /> */}
    </RetinaFormBuilder>
  );
};
